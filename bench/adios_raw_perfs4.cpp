...
static std::vector<int64_t>                  data_ids, offset_ids;
...
void write (char* buffer){
  ...
  for (int i = 0 ; i < splitting; i++) {
    ...
    adios_write_byid (adios_handle, offset_ids[i],      &offset);
    adios_write_byid (adios_handle, data_ids[i],        &buffer[offset]);
  }
}
...
void initAdios (const char* method, uint64_t max_buffer_size) {
  ...
  for (int i = 0; i < splitting; i++) {
    offset_ids.push_back  ( adios_define_var ( adios_group_id, "offset", "", adios_integer, 0,0,0));
    data_ids.push_back    ( adios_define_var ( adios_group_id, "data",   "", adios_byte, "batch_size", "global_size", "offset"));
  }
}
